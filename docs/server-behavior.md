...
**`--limit-concurrency` is an application-level gate.** When a request's headers have been fully received, Uvicorn checks whether the number of open connections *or* the number of in-flight request/response tasks has reached the limit. If so, it responds immediately with a "503 Service Unavailable" - the request is **not** queued and does **not** wait for a slot to free up. The connection serving the current request is itself counted, so a value of `N` allows up to `N - 1` *other* concurrent requests; in particular a value of `1` refuses every request. Because the check happens only after the connection has been accepted and its headers read, `--limit-concurrency` bounds concurrent *requests*, not the number of open *connections* - idle keep-alive connections can still accumulate.
...
To make the distinction concrete, consider `--limit-concurrency 5` with a slow application and 10 simultaneous requests: 5 requests are processed and the other 5 receive a 503 essentially immediately. They are not held in the backlog and processed later. To instead *queue* excess load, put a reverse proxy (e.g. nginx) in front of Uvicorn, or scale out with more workers.
