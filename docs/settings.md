...
* `--root-path <str>` - Serve the application under the provided root path (mount point). This prefixes incoming request paths before they reach the application. **Default:** *""*.
* `--asgi-root-path <str>` - Set the ASGI `root_path` without prefixing incoming request paths. Useful when a reverse proxy serves the app under a subpath. Mutually exclusive with `--root-path`. **Default:** *""*.
