...
* `--http <str>` - Set the [HTTP protocol implementation](concepts/http-protocols.md). The httptools implementation provides greater performance, but is not compatible with PyPy. The httpunk and zttp implementations are experimental and require their respective packages. **Options:** *'auto', 'h11', 'httptools', 'httpunk', 'zttp'.* **Default:** *'auto'*.
* `--http2` - Enable [HTTP/2](concepts/http2.md). This requires `--http httpunk` or `--http zttp`. **Default:** *False*.
